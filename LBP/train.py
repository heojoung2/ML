...
batch_size=20
def neighbor_function(img, x, y):
    result =[]

    result.append(img[x - 1,y - 1])
    result.append(img[x , y - 1])
    result.append(img[x + 1, y - 1])
    result.append(img[x + 1, y])
    result.append(img[x + 1, y + 1])
    result.append(img[x, y + 1])
    result.append(img[x - 1, y + 1])
...
def lbp_function(file_name):
    im = cv2.imread(file_name,0)   #1-> 그대로 가져오겠다, 0 -> gray스케일로 바꾸겠다
    im = cv2.resize(im, (70,134), interpolation=cv2.INTER_AREA)
    rows, cols = im.shape
    lbp_img = np.zeros(((rows-1)//3, (cols-1)//3), np.uint8)

    for x in range(1,rows-1,3):
        for y in range(1,cols-1,3):
            center = im[x,y]
            neighbor = neighbor_function(im, x, y)

            values = thresholded(center, neighbor)
            ...
            lbp_img.itemset((x//3,y//3),result)
    return lbp_img
...
def read_image(something):
    ...
    if something == 'train':
        ...
        for name in pos_train_images:
            im = lbp_function(pos_train_path + name)
            image.append(im/255)
            label.append([0,1])

        for name in neg_train_images:
            im = lbp_function(neg_train_path + name)
            image.append(im/255)
            label.append([1,0])
        ...
        for name in pos_test_images:
            im = lbp_function(pos_test_path + name)
            image.append(im/255)
            label.append([0,1])

        for name in neg_test_images:
            im = lbp_function(neg_test_path + name)
            image.append(im/255)
            label.append([1,0])

    return np.array(image), np.array(label)

train_image, train_label = read_image('train')  # 1200
#test_image, test_label = read_image('test')     #291

x_input = tf.placeholder(tf.float32, shape=[None,44,23])
y_input = tf.placeholder(tf.float32, shape=[None,2])
keep_prob = tf.placeholder(tf.float32)

x_image = tf.reshape(x_input,[-1,44,23,1])  #55x40
...
#fully connected
w4 = tf.get_variable("w4", shape=[6*3*128, 1024],initializer=tf.contrib.layers.xavier_initializer())
b4 =  tf.Variable(tf.constant(0.1, shape=[1024]))

h4 = tf.reshape(h3,[-1,6*3*128])
...
train_batch_x, train_batch_y = tf.train.shuffle_batch([train_image,train_label],batch_size=batch_size,capacity=1200,min_after_dequeue=1000,enqueue_many=True)
#test_batch_x, test_batch_y = tf.train.batch([test_image,test_label],batch_size=batch_size+12,enqueue_many=True,capacity=1200)
...
'''
for i in range(0,len(test_image),batch_size+12):
